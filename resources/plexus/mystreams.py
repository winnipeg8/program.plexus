...
def my_streams_menu():
    if not os.path.exists(mystrm_folder):
        xbmcvfs.mkdir(mystrm_folder)
    ...
    addDir('[B][COLOR maroon]' + translate(30009) + '[/COLOR][/B]', MainURL, 11,
           os.path.join(addonpath, art, 'plus-menu.png'), 1, False)
...
def add_stream(name='', url='', iconimage=''):
    if not name or not url:
                    ...
                    if (keyb.isConfirmed()):
                        ...
                        if name == '':
                            ...
                            xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % (
                                translate(30000), translate(30015), 1, os.path.join(addonpath, "icon.png")))
        ...
        save(os.path.join(mystrm_folder, filename), content)
        xbmc.executebuiltin(
            "Notification(%s,%s,%i,%s)" % (translate(30000), translate(30015), 1, os.path.join(addonpath, "icon.png")))
        xbmc.executebuiltin("Container.Refresh")
...
def remove_stream(name, url):
    ...
    try:
        os.remove(ficheiro)
        xbmc.executebuiltin(
            "Notification(%s,%s,%i,%s)" % (translate(30000), translate(30016), 1, os.path.join(addonpath, "icon.png")))
        xbmc.executebuiltin("Container.Refresh")
    except:
        pass
