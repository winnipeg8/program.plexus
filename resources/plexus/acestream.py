...
def load_local_torrent():
    ...
    if torrent_file:
        if xbmc.getCondVisibility('system.platform.windows'):
            ...
            acestreams("Local .torrent (" + str("file://" + torrent_file) + ")", "",
                       'file://' + urllib.quote(torrent_file))
    else:
        pass
...
def acestreams(name, iconimage, chid):
    if not iconimage:
        ...
        iconimage = urllib.unquote(iconimage)
    if settings.getSetting('addon_history') == "true":
        try:
            add_to_history(name, str(chid), 1, iconimage)
        except:
            pass
    if settings.getSetting('engine_app') != '2' and settings.getSetting('engine_app') != '3':
        ...
        if '.acelive' in chid:
            pass
        elif '.torrent' in chid:
            pass
        else:
            if 'acestream://' in chid:
                pass
            else:
                chid = 'acestream://' + chid
        if settings.getSetting('engine_app') == '2':
            xbmc.executebuiltin(
                'XBMC.StartAndroidActivity("org.acestream","android.intent.action.VIEW","","' + chid + '")')
        elif settings.getSetting('engine_app') == '3':
            xbmc.executebuiltin(
                'XBMC.StartAndroidActivity("ru.vidsoftware.acestreamcontroller.free","android.intent.action.VIEW","","'
                + chid + '")')
...
def acestreams_builtin(name, iconimage, chid):
    if xbmc.getCondVisibility('system.platform.windows'):
        try:
            ...
            subprocess.Popen("wmic process where ExecutablePath='" + needed_value.replace('\\', '\\\\') + "' delete",
                             shell=True)
            xbmc.sleep(200)
