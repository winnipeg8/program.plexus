    ...
    if version_source:
        ...
        if xbmc.getCondVisibility('system.platform.linux') and not xbmc.getCondVisibility('system.platform.Android'):
            ...
            if os.uname()[4] == "i386" or os.uname()[4] == "i686":
                ...
                platf = "osx64"
        try:
            if version_source["sopcast"][platf] != settings.getSetting('sopcast_version'):
                configure_sopcast(
                    version_source["sopcast"][platf])
            sopcast_update = True
        except:
            sopcast_update = False
        try:
            if version_source["acestream"][platf] != settings.getSetting('acestream_version'):
                configure_acestream(
                    version_source["acestream"][platf])
            acestream_update = True
        except:
            acestream_update = False
        if acestream_update and sopcast_update:
            settings.setSetting('last_version_check', value=versao)
        return
    ...
    if xbmc.getCondVisibility('system.platform.linux') and not xbmc.getCondVisibility('system.platform.Android'):
            ...
            if not sopcast_executable:
                ...
                for i in range(1024):
                    try:
                        ...
                        print(asubkey)
                        aKeydois = _winreg.OpenKey(aReg, os.path.join(
                            'SOFTWARE\Microsoft\Windows NT\CurrentVersion\ProfileList', asubkey))
                ...
                if not users:
                    ...
                    download_tools().Downloader(srvany_executable, srvanytgz_download_location, translate(30087),
                                                translate(30000))
                    xbmc.sleep(1000)
                    ...
                    xbmc.sleep(1000)
                    cmd = ['sc', 'create', 'sopcastp2p', 'binpath=',
                           os.path.join(os.path.join(sopcast_executable.replace("SopCast.exe", "")), 'srvany.exe')]
                    proc = subprocess.Popen(cmd, stdout=subprocess.PIPE, shell=True)
                    ...
                    if not servicecreator:
                        ...
                        try:
                            ...
                            _winreg.SetValueEx(key, 'AppDirectory', 0, _winreg.REG_SZ,
                                               os.path.join(sopcast_executable.replace("SopCast.exe", "")))
                            _winreg.SetValueEx(key, 'Application', 0, _winreg.REG_SZ,
                                               os.path.join(os.path.join(sopcast_executable.replace("SopCast.exe", "")),
                                                            "SopCast.exe"))
                        ...
                        if not regedit:
                            ...
                            if len(lines) != 1:
                                ...
                                for line in proc.stdout:
                                    ...
                                    permissions = True
                                if not permissions:
                                    mensagemok(translate(30000), translate(30095))
                                else:
                                    ...
                                    download_tools().Downloader(srvany_permissions,
                                                                os.path.join(pastaperfil, "sopcastp2p-permissions.txt"),
        ...
        if found:
            ...
            if os.path.exists(xbmc_data_path) and uid == os.stat(xbmc_data_path).st_uid:
                ...
                if not os.path.exists(android_binary_path) or os.path.getsize(android_binary_path) != os.path.getsize(
        ...
        if found:
            ...
            if not opcao:
                ...
                if os.path.exists(os.path.join("sdcard", "Download")):
                    pasta = os.path.join("sdcard", "Download")
                    if "arm" in os.uname()[4]:
                        acefile = os.path.join("sdcard", "Download", acestreamengine_apk_arm.split("/")[-1])
                    else:
                        acefile = os.path.join("sdcard", "Download", acestreamengine_apk_x86.split("/")[-1])
                else:
                    dialog = xbmcgui.Dialog()
                    pasta = dialog.browse(int(0), translate(30105), 'myprograms')
                    if "arm" in os.uname()[4]:
                        acefile = os.path.join(pasta, acestreamengine_apk_arm.split("/")[-1])
                    else:
                        acefile = os.path.join(pasta, acestreamengine_apk_x86.split("/")[-1])
                if "arm" in os.uname()[4]:
                    download_tools().Downloader(acestreamengine_apk_arm, acefile, translate(30117), translate(30000))
                else:
                    download_tools().Downloader(acestreamengine_apk_x86, acefile, translate(30117), translate(30000))
            ...
            if opcao:
                if os.path.exists(os.path.join("sdcard", "Download")):
                    pasta = os.path.join("sdcard", "Download")
                    if "arm" in os.uname()[4]:
                        acefile = os.path.join("sdcard", "Download", android_aceplayer_arm.split("/")[-1])
                    else:
                        os.path.join("sdcard", "Download", android_aceplayer_x86.split("/")[-1])
                else:
                    dialog = xbmcgui.Dialog()
                    pasta = dialog.browse(int(0), translate(30105), 'myprograms')
                    if "arm" in os.uname()[4]:
                        ...
                        acefile = os.path.join(pasta, acestreamengine_apk_x86.split("/")[-1])
                if "arm" in os.uname()[4]:
                    download_tools().Downloader(android_aceplayer_arm, acefile, translate(30124), translate(30000))
                else:
                    download_tools().Downloader(android_aceplayer_x86, acefile, translate(30124), translate(30000))
                if tarfile.is_tarfile(acefile):
                    download_tools().extract(acefile, pasta)
                    download_tools().remove(acefile)
                xbmc.sleep(2000)
