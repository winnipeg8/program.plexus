...
class _TSPlayer(xbmc.Player):
    def __init__(self):
        ...
        self.duration = None
        self.coms = []
        if settings.getSetting('force_dvplayer') == 'true':
...
class TSengine():
    ...
    def getosxPort(self):
        try:
            path = os.path.join('/Applications', 'Ace Stream.app', 'Contents', 'Resources', 'wineprefix', 'drive_c',
                                'users', 'IGHOR', 'Application Data', 'ACEStream', 'engine')
    ...
    def startosx(self):
        ...
        import subprocess
        comd = [os.path.join('/Applications', 'Ace Stream.app', 'Contents', 'Resources', 'Wine.bundle', 'Contents',
                             'Resources', 'bin', 'wine'),
                os.path.join('/Applications', 'Ace Stream.app', 'Contents', 'Resources', 'wineprefix', 'drive_c',
                             'users', 'IGHOR', 'Application Data', 'ACEStream', 'engine', 'ace_engine.exe')]
        print comd
        try:
        ...
        if xbmc.getCondVisibility('System.Platform.Android'):
            try:
                if settings.getSetting('engine_app') == "1":
                    ...
                    command = ["sh", "/data/data/" + settings.getSetting(
                        'app_id') + "/files/program.plexus/org.acestream.engine/files/droidace.sh",
                               settings.getSetting('app_id')]
            ...
            if "arm" in os.uname()[4]:
                ...
                if settings.getSetting('ace_cmd') == "0":
                    ...
                    if settings.getSetting('total_max_upload_rate') != "0":
                        ...
                        command.append(settings.getSetting('total_max_upload_rate'))
                    print command
                elif settings.getSetting('ace_cmd') == "1":
                    command = ["acestreamengine", "--client-console"]
                elif settings.getSetting('ace_cmd') == "2":
                    command = settings.getSetting('ace_cmd_alternative').split(' ')
                try:
                    self.proc = subprocess.Popen(command)
                except:
                    self.sm('Not Installed')
    ...
    def get_link(self, index=0, title='', icon='', thumb=''):
        ...
        for k, v in self.files.iteritems():
            if v == index:
                self.filename = urllib.unquote(k).replace('/', '_').replace('\\', '_')
        try:
    ...
    def play_url_ind(self, index=0, title='', icon='', thumb=''):
        ...
        if settings.getSetting('aceplay_type') == str(1):
            if ":6878/" in self.lnk:
                self.lnk = self.lnk.replace(":6878", ":" + settings.getSetting('playerport'))
        if self.progress:
            self.progress.close()
        ...
        if self.local:
            ...
            while self.player.active and not self.local:
                if settings.getSetting('engine-status') == "true":
                    if not show_window and xbmc.getCondVisibility('Window.IsActive(videoosd)'):
                        lat123.show()
                        show_window = True
    ...
    def loop(self):
        ...
        if self.player.isPlaying():
            if self.player.getTotalTime() > 0:
                cpos = int(
                    (1 - (self.player.getTotalTime() - self.player.getTime()) / self.player.getTotalTime()) * 100)
            else:
                cpos = 0
            if cpos in pos:
                pos.remove(cpos)
...
class TSServ(threading.Thread):
    ...
    def exec_com(self):
        ...
        if comm == 'HELLOTS':
            ...
            if 'stream=1' in self.params:
                self.log.out('Live Stream')
            else:
                self.log.out('VOD Stream')
        elif comm == 'RESUME':
            self.pause = 0
        elif comm == 'PAUSE':
            self.pause = 1
    ...
    def showStats(self, params):
        ...
        if st == 'idle':
            ...
            engine_data = {"action": str(translate(30059)), "percent": str(params.split(';')[1]) + "%",
                           "download": str(params.split(';')[5]) + " Kb/s",
                           "upload": str(params.split(';')[7]) + " Kb/s", "seeds": str(params.split(';')[8]),
                           "total_download": str(int(params.split(';')[10]) / (1024 * 1024)) + 'Mb',
                           "total_upload": str(int(params.split(';')[12]) / (1024 * 1024)) + 'Mb'}
            if settings.getSetting('debug_mode') == "true":
            ...
            engine_data = {"action": str(translate(30060)), "percent": str(params.split(';')[1]) + "%",
                           "download": str(params.split(';')[3]) + " Kb/s",
                           "upload": str(params.split(';')[5]) + " Kb/s", "seeds": str(params.split(';')[6]),
                           "total_download": str(int(params.split(';')[8]) / (1024 * 1024)) + 'Mb',
                           "total_upload": str(int(params.split(';')[10]) / (1024 * 1024)) + 'Mb'}
            if settings.getSetting('engine-status') == "true":
                try:
            ...
            engine_data = {"action": str(translate(30061)), "percent": str(params.split(';')[1]) + "%",
                           "download": str(params.split(';')[5]) + " Kb/s",
                           "upload": str(params.split(';')[7]) + " Kb/s", "seeds": str(params.split(';')[8]),
                           "total_download": str(int(params.split(';')[10]) / (1024 * 1024)) + "Mb",
                           "total_upload": str(int(params.split(';')[12]) / (1024 * 1024)) + "Mb"}
...
def ace_control_thread():
    while json.loads(\
            xbmc.executeJSONRPC('{"jsonrpc":"2.0","method":"Player.GetActivePlayers","params":{},"id":3}'))["result"]:
        xbmc.sleep(500)
    stop_aceengine()
...
def stop_aceengine():
    if settings.getSetting('shutdown-engine') == 'true' and settings.getSetting('kill_type') == '0':
        if xbmc.getCondVisibility('system.platform.windows'):
                ...
                try:
                    ...
                    if 'arm' not in os.uname()[4]:
                        if settings.getSetting('acestream_cachefolder') == '':
                            acestream_cachefolder_file = os.path.join(os.getenv("HOME"), '.ACEStream', 'cache',
                                                                      '.acestream_cache')
                        else:
                            acestream_cachefolder_file = settings.getSetting('acestream_cachefolder')
                    else:
                        if settings.getSetting('acestream_cachefolder') == '':
                            acestream_cachefolder_file = os.path.join(os.getenv("HOME"), '.ACEStream', 'cache')
                        else:
            ...
            try:
                kill_cmd = [
                    os.path.join('/Applications', 'Ace Stream.app', 'Contents', 'Resources', 'Wine.bundle', 'Contents',
                                 'Resources', 'bin', 'wine'),
                    os.path.join('/Applications', 'Ace Stream.app', 'Contents', 'Resources', 'wineprefix', 'drive_c',
                                 'windows', 'system', 'taskkill.exe'), '/f', '/im', 'ace_engine.exe']
            ...
            if settings.getSetting('save') != "true":
                try:
                    if settings.getSetting('acestream_cachefolder') != '':
                        dirs, cache_files = xbmcvfs.listdir(
                            os.path.join(settings.getSetting('acestream_cachefolder'), '.acestream_cache'))
...
class OverlayText(object):
    def __init__(self):
        ...
        self._background = xbmcgui.ControlImage(origin_x, origin_y, window_w, window_h,
                                                os.path.join(addonpath, "resources", "art", "background.png"))
