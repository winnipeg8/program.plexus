        ...
        if not config:
            ...
            try:
                ...
                if xbmcvfs.exists(codec_file):
                    xbmcvfs.rename(codec_file, \
                                   os.path.join(os.path.join(value.replace("SopCast.exe", "")), 'codec', 'sop.ocx.old'))
            except:
                pass
            aReg = _winreg.ConnectRegistry(None, _winreg.HKEY_LOCAL_MACHINE)
            ...
            try:
                ...
                if xbmcvfs.exists(codec_file):
                    xbmcvfs.rename(codec_file, \
                                   os.path.join(os.path.join(value.replace("SopCast.exe", "")), 'codec', 'sop.ocx'))
            except:
                pass
...
class streamplayer(xbmc.Player):
    ...
    def onPlayBackStarted(self):
        mensagemprogresso.close()
        if xbmc.Player(xbmc.PLAYER_CORE_AUTO).getPlayingFile() != "http://" + LOCAL_IP + ":" + str(
