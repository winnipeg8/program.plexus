...
def addDir(name, url, mode, iconimage, total, pasta, fan_art="%s/fanart.jpg" % settings.getAddonInfo("path")):
    ...
    if mode == 1 or mode == 2:
        ...
        if os.path.exists(os.path.join(mystrm_folder, fic)):
            contextmen.append((translate(30025), 'XBMC.RunPlugin(%s?mode=13&url=%s&name=%s&iconimage=%s)' % (
                sysargv, urllib.quote_plus(url), name, iconimage)))
        else:
            contextmen.append((translate(30026), 'XBMC.RunPlugin(%s?mode=12&url=%s&name=%s&iconimage=%s)' % (
                sysargv, urllib.quote_plus(url), name, iconimage)))
